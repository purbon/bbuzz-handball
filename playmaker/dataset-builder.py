...
def extract_coordinates_from(df, io_path="dumps/"):
    ...
    columns_to_remove = ["Unnamed: 14",
                         "POS. B", "POS. B.1", "analysis", "score_diff", "live_possession_duration_in_sec",
    ...
    columns_to_remove += [f"prev{i}_possession_result" for i in range(1, 6, 1)]
    columns_to_remove += [f"prev{i}_score_diff" for i in range(1, 6, 1)]
